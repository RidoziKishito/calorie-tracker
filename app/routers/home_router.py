...
from app.deps import get_optional_user, get_food_log_service, get_health_repository, get_auth_service, get_personal_food_service
...
async def meals_page(
    ...
    dates = [entry.updated_at.strftime('%d/%m') if entry.updated_at else '' for entry in weight_history]
...
async def dashboard(request: Request, user=Depends(get_optional_user), food_log_service: FoodLogService = Depends(get_food_log_service)):
    if not user:
        return RedirectResponse(url="/account/login", status_code=303)
...
async def manual_input_page(request: Request, user=Depends(get_optional_user), personal_food_service: PersonalFoodService = Depends(get_personal_food_service)):
    if not user:
        return RedirectResponse(url="/account/login", status_code=303)
...
async def add_manual_meal(
    ...
    food_log_service.add_food_log(user.id, final_name, final_calories, carbs, protein, fat, meal_type)
...
async def update_health_page(request: Request, user=Depends(get_optional_user)):
    if not user:
        return RedirectResponse(url="/account/login", status_code=303)
    return templates.TemplateResponse("update_health.html", {"request": request, "user": user})
...
async def update_health(
    ...
    try:
        ...
        return templates.TemplateResponse("update_health.html", {
            "request": request, 
            "user": user,
            "error": str(e)
        })
...
@router.get("/diary")
async def diary(request: Request, date: Optional[str] = None, user=Depends(get_optional_user), food_log_service: FoodLogService = Depends(get_food_log_service)):
    if not user:
        return RedirectResponse(url="/account/login", status_code=303)
...
async def create_meal_page(request: Request, user=Depends(get_optional_user)):
    if not user:
        return RedirectResponse(url="/account/login", status_code=303)
    return templates.TemplateResponse("create_meal.html", {"request": request, "user": user})
...
async def edit_personal_food_page(
    ...
    return templates.TemplateResponse("edit_meal.html", {
        "request": request,
        "user": user,
        "food": food
    })
...
async def meal_detail(request: Request, id: int = Query(...), user=Depends(get_optional_user), food_log_service: FoodLogService = Depends(get_food_log_service)):
    if not user:
        return RedirectResponse(url="/account/login", status_code=303)
    food_log = food_log_service.get_food_log_by_id(id)
    if not food_log or food_log.user_id != user.id:
        return RedirectResponse(url="/home/dashboard", status_code=303)
    return templates.TemplateResponse("meal_detail.html", {"request": request, "user": user, "food_log": food_log})
...
async def report_meal(
    ...
    print(f"Meal report from user {user.id} - Meal ID: {id}, Suggested name: {correct_name}")
    return RedirectResponse(url="/home/diary", status_code=303)

@router.get("/profile")
async def profile(request: Request, user=Depends(get_optional_user), food_log_service: FoodLogService = Depends(get_food_log_service)):
    if not user:
        return RedirectResponse(url="/account/login", status_code=303)
...
async def upload_avatar(
    ...
    image_url = CloudinaryService.upload_image(file_content, folder="nutrition_tracker/avatars")
