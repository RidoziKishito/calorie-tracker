...
async def camera_scan_page(request: Request, user=Depends(get_optional_user)):
    ...
    return templates.TemplateResponse("camera_scan.html", {"request": request, "user": user})


@router.get("/result")
async def camera_result_page(request: Request, result: str = "", user=Depends(get_optional_user)):
    if not user:
        return RedirectResponse(url="/account/login", status_code=303)
    return templates.TemplateResponse("your_meal.html", {
        "request": request, 
        "user": user, 
...
async def upload_image(
    ...
    try:
        ...
        image_url = CloudinaryService.upload_image(content, folder="nutrition_tracker/food_logs")
        ...
        if predicted_label:
            ...
            if food is None:
                not_found = True
                print(f"Food not found in database for ai_slug: {predicted_label}")
        else:
            not_found = True
