...
async def login_submit(
    ...
    try:
        ...
        # Log lỗi một cách chuẩn, không hiển thị traceback cho client
...
async def register_submit(
    ...
    if password != confirm_password:
        return templates.TemplateResponse(
            "register.html",
            {"request": request, "error": "Mật khẩu xác nhận không khớp!"},
        )
...
async def onboarding_page(request: Request):
    ...
    return templates.TemplateResponse(
        "onboarding.html", {"request": request, "email": email}
    )
...
async def onboarding_submit(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "onboarding.html", {"request": request, "email": email, "error": str(e)}
        )
    except Exception as e:
        return templates.TemplateResponse(
            "onboarding.html",
            {"request": request, "email": email, "error": "Lỗi xử lý onboarding."},
        )
...
async def sync_supabase_auth(
    ...
    try:
        ...
        if not access_token or not user_data:
            return JSONResponse(
                {"error": "Missing authentication data"}, status_code=400
            )
        ...
        try:
            ...
            # Không trả thông tin chi tiết token verify cho client
            logger.warning("Token verification error: %s", verify_error)
        ...
        response = JSONResponse(
            {
                ...
                "redirect_url": (
                    "/account/onboarding" if needs_onboarding else "/home/dashboard"
                ),
            }
        )
        ...
        # Ghi log chi tiết ở server, trả thông báo chung cho client
        logger.error("Supabase sync error: %s", e, exc_info=True)
        return JSONResponse({"error": "Đã xảy ra lỗi hệ thống."}, status_code=500)
