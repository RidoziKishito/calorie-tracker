...
class AuthService:
    ...
    def _calculate_metrics(self, weight: float, height: float, dob: date, gender: GenderEnum, activity_level: str) -> dict:
        ...
        activity_keys = {
            "low": "Sedentary", "sedentary": "Sedentary",
        ...
        if isinstance(activity_level, ActivityLevelEnum):
             ...
             normalized_level = activity_keys.get(str(activity_level).lower(), "Sedentary")
        ...
        return {
            "bmi": bmi,
            "tdee": tdee,
            "activity_enum": act_enum
        }

    def complete_onboarding(self, email: str, dob_str: str, gender: str, 
                            weight: float, height: float, activity_level: str):
        ...
        metrics = self._calculate_metrics(weight, height, user.dob, user.gender, activity_level)
    ...
    def update_health(self, user_id: int, weight: float, height: float):
        ...
        current_activity = latest_health.activity_level if latest_health else "Sedentary"
        
        metrics = self._calculate_metrics(weight, height, user.dob, user.gender, current_activity)
    ...
    def sync_supabase_user(self, email: str, full_name: str, supabase_user_id: str = None):
        """
